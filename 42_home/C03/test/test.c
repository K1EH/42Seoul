#include <unistd.h> // Include the correct header for write() function
#include <stdio.h> // Include this header for NULL definition

void    print_hex_addr(char *addr)
{
    unsigned long long int addr1; // Use the correct data type
    int    i;
    char        *hexa;
    char        tmp[16];

    addr1 = (unsigned long long int)addr;
    hexa = "0123456789abcdef";
    i = 15;
    while (i >= 0)
    {
        tmp[i] = hexa[(unsigned long long int)addr1 % 16];
        addr1 /= 16;
        i--;
    }
    write(1, tmp, 16); // Write 16 characters for address
    write(1, ": ", 2);
}
...
void    *ft_print_memory(void *addr, unsigned int size)
{
    ...
    while (size > 0)
    {
        ...
        if (size < 16)
        {
            // Print extra spaces if there are fewer than 16 bytes left
            unsigned int spaces = 16 - size;
            while (spaces > 0)
            {
                write(1, "   ", 3);
                if (spaces % 2 == 0)
            ...
            write(1, " ", 1); // Additional space after the 8th character
        }
        if (size < 16)
            size = 0;
        else
